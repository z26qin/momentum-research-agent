"""Session gap ledger: compile verdicts + engine/search traces for replay.

Cross-session persistence still lives in reports/gap_ledger.jsonl.
This module fills verification.json gaps[] / traces[] so a session can be
re-read without a second research loop.
"""
...
def replay_trace(trace: ToolTrace) -> dict[str, Any]:
    ...
    try:
        stored = json.loads(trace.observation)
    except json.JSONDecodeError:
        stored = None
    if isinstance(stored, dict) and "delivery_contract" in stored:
        live = attach_delivery_contract(
            live, requested_end=None if end is None else str(end)
        )
